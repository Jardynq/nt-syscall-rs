...
/*
    Enter x64 mode on an x86 thread.
    The thread must previously have entered x86 mode via x64::enter_x86_with_stack().
*/
pub macro enter_x64_restore_stack() {
    concat!(x86::enter_x64!(), x64::epilogue!())
}
...
pub macro get_cpu_mode() {
    concat!(
        ...
        x86::assemble!("mov word ptr [edx], ax"),
        x86::arg_next!(1),
    )
}
