...
pub macro callconv_varargs {
    ...
    (@arg ($($count:tt)+), ptr $(, $($tail:tt)* )?) => { x64::callconv_varargs!(@arg ($($count)+), u64 $(, $($tail)* )?) },

    (@arg ($($count:tt)+) $(,)?) => { x64::callconv_varargs!(@stack ($($count)+)) },
    ...
    ($($args:tt)*) => {
        x64::callconv_varargs!(@arg (0), $($args)*)
    },
}
